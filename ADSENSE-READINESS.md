...
- **Spotify embeds are now click-to-load.** The iframe used to load on every
  message page, handing Spotify the reader's IP and cookies before anyone
  agreed to anything.
...
1. ~~The home page advertises features that no longer exist.~~ **Resolved.** The
   ...
   Still worth knowing: `SendMessageForm.tsx` has no photo, sketch or song UI —
   it passes `undefined` for `photo` and `spotifyTrackId` and offers only text
   plus a time-capsule date. The backend, the message renderer and the Spotify
   API routes are all still in place, so restoring the feature is UI work only.
   The unused copy for it is still in `site-content.json`
   (`sendAttachPhotoButton`, `sendDrawButton`, `sendAddSongButton`, …); it is
   not rendered anywhere, so it is harmless, and it is the config you would
   need if you bring the feature back.
