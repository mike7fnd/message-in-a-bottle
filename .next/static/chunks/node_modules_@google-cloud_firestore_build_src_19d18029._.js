 ...
 */ class ClientPool {
     ...
     */ acquire(requestTag, requiresGrpc) {
        ...
        if (selectedClient) {
            ...
            (0, logger_1.logger)("ClientPool[".concat(this.instanceId, "].acquire"), requestTag, 'Re-using existing client [%s] with %s remaining operations', selectedClientId, this.concurrentOperationLimit - selectedClientRequestCount);
        } else {
            const newClientId = 'cli' + (0, util_1.requestTag)();
            (0, logger_1.logger)("ClientPool[".concat(this.instanceId, "].acquire"), requestTag, 'Creating a new client [%s] (requiresGrpc: %s)', newClientId, requiresGrpc);
     ...
     */ async release(requestTag, client) {
        ...
        (0, logger_1.logger)("ClientPool[".concat(this.instanceId, "].release"), requestTag, 'Releasing client [%s] (gc=%s)', clientId, gcDetermination);
        if (!gcDetermination.shouldGarbageCollectClient) {
            return;
        }
        (0, logger_1.logger)("ClientPool[".concat(this.instanceId, "].release"), requestTag, 'Garbage collecting client [%s] (%s)', clientId, this.lazyLogStringForAllClientIds);
        const activeClientDeleted = this.activeClients.delete(client);
        this.failedClients.delete(client);
        await this.clientDestructor(client);
        (0, logger_1.logger)("ClientPool[".concat(this.instanceId, "].release"), requestTag, 'Garbage collected client [%s] activeClientDeleted=%s (%s)', clientId, activeClientDeleted, this.lazyLogStringForAllClientIds);
    }
    /**
    ...
    async terminate() {
        ...
        if (this.opCount > 0) {
            (0, logger_1.logger)("ClientPool[".concat(this.instanceId, "].terminate"), /* requestTag= */ null, 'Waiting for %s pending operations to complete before terminating (%s)', this.opCount, this.lazyLogStringForAllClientIds);
            await this.terminateDeferred.promise;
        }
        (0, logger_1.logger)("ClientPool[".concat(this.instanceId, "].terminate"), /* requestTag= */ null, 'Closing all active clients (%s)', this.lazyLogStringForAllClientIds);
 ...
 */ class LazyLogStringForAllClientIds {
    toString() {
        const activeClientsDescription = Array.from(this.activeClients.entries()).map((param)=>{
            let [client, metadata] = param;
            return "".concat(this.clientIdByClient.get(client), "=").concat(metadata.activeRequestCount);
        }).sort().join(', ');
        const failedClientsDescription = Array.from(this.failedClients).map((client)=>"".concat(this.clientIdByClient.get(client))).sort().join(', ');
        return "".concat(this.activeClients.size, " active clients: {") + activeClientsDescription + '}, ' + "".concat(this.failedClients.size, " failed clients: {") + failedClientsDescription + '}';
    }
    constructor(config){
 ...
 */ class BaseShouldGarbageCollectClientResult {
     ...
     */ toString() {
        ...
        for (const propertyName of Object.getOwnPropertyNames(this)){
            ...
            propertyStrings.push("".concat(propertyName, "=").concat(propertyValue));
        }
