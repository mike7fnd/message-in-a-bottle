...
class GoogleErrorDecoder {
    ...
    decodeHTTPError(json) {
        ...
        if (!errorMessage) {
            throw new Error("Received error message ".concat(json, ", but failed to serialize as proto3 message"));
        }
...
function words(str) {
    let normalize = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false;
    if (normalize) {
...
function createByteLengthFunction(message) {
    return function getByteLength(obj) {
        try {
            ...
            (0, warnings_1.warn)('error_encoding_protobufjs_object', "Cannot encode protobuf.js object: ".concat(stringified, ": ").concat(err));
     ...
     */ return (argument, callback)=>{
        ...
        /** Repeat the API call as long as necessary. */ function repeat(err) {
            ...
            if (deadline && now.getTime() >= deadline) {
                const error = new googleError_1.GoogleError("Total timeout of API ".concat(apiName, " exceeded ").concat(retry.backoffSettings.totalTimeoutMillis, " milliseconds ").concat(err ? "retrying error ".concat(err, " ") : '', " before any response was received."));
                error.code = status_1.Status.DEADLINE_EXCEEDED;
                callback(error);
                return;
            }
            if (retries && retries >= maxRetries) {
                const error = new googleError_1.GoogleError('Exceeded maximum number of retries ' + (err ? "retrying error ".concat(err, " ") : '') + 'before any response was received');
                error.code = status_1.Status.DEADLINE_EXCEEDED;
                callback(error);
                return;
            }
...
class StreamProxy extends duplexify {
     ...
     */ throwIfMaxRetriesOrTotalTimeoutExceeded(deadline, maxRetries, totalTimeoutMillis, originalError, originalTimeout, retries) {
        ...
        if (originalTimeout && (totalTimeoutMillis === 0 || totalTimeoutMillis < 0 || deadline && nowTime >= deadline)) {
            const error = new googleError_1.GoogleError("Total timeout of API exceeded ".concat(originalTimeout, " milliseconds ").concat(originalError ? "retrying error ".concat(originalError, " ") : '', " before any response was received."));
            error.code = status_1.Status.DEADLINE_EXCEEDED;
            throw error;
        }
        if (maxRetries === 0) {
        ...
        if (retries && retries >= maxRetries) {
            const error = new googleError_1.GoogleError('Exceeded maximum number of retries ' + (originalError ? "retrying error ".concat(originalError, " ") : '') + 'before any response was received');
            error.code = status_1.Status.DEADLINE_EXCEEDED;
            throw error;
        }
    }
     ...
     */ setStream(apiCall, argument) {
        let retryRequestOptions = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {}, retry = arguments.length > 3 ? arguments[3] : void 0;
...
class StreamingApiCaller {
    ...
    wrap(func) {
        switch(this.descriptor.type){
            ...
            default:
                (0, warnings_1.warn)('streaming_wrap_unknown_stream_type', "Unknown stream type: ".concat(this.descriptor.type));
        }
        return func;
    }
 ...
 */ class BundleExecutor {
     ...
     */ schedule(apiCall, request, callback) {
        ...
        if (bundleId === undefined) {
            (0, warnings_1.warn)('bundling_schedule_bundleid_undefined', 'The request does not have enough information for request bundling. ' + "Invoking immediately. Request: ".concat(JSON.stringify(request), " ") + "discriminator fields: ".concat(this._descriptor.requestDiscriminatorFields));
            return apiCall(request, callback);
        }
        if (request[this._descriptor.bundledField] === undefined) {
            (0, warnings_1.warn)('bundling_no_bundled_field', "Request does not contain field ".concat(this._descriptor.bundledField, " that must present for bundling. ") + "Invoking immediately. Request: ".concat(JSON.stringify(request)));
            return apiCall(request, callback);
        }
     ...
     */ _runNow(bundleId) {
        if (!(bundleId in this._tasks)) {
            (0, warnings_1.warn)('bundle_runnow_bundleid_unknown', "No such bundleid: ".concat(bundleId));
            return;
        }
        this._maybeClearTimeout(bundleId);
        const task = this._tasks[bundleId];
...
function getField(request, field) {
    let allowObjects = arguments.length > 2 && arguments[2] !== void 0 // in most cases, we need leaf fields
     ? arguments[2] : false;
...
function deepCopyWithoutMatchedFields(request, fieldsToSkip) {
    let fullNamePrefix = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : '';
    if (typeof request !== 'object' || request === null) {
        return request;
    }
    const copy = Object.assign({}, request);
    for(const key in copy){
        if (fieldsToSkip.has("".concat(fullNamePrefix).concat(key))) {
            delete copy[key];
            continue;
        }
        const nextFullNamePrefix = "".concat(fullNamePrefix).concat(key, ".");
 ...
 */ class OperationsClient {
    ...
    operationsProtos, options){
        ...
        for (const methodName of operationsStubMethods){
            const innerCallPromise = this.operationsStub.then((stub)=>function() {
                    for(var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++){
                        args[_key] = arguments[_key];
                    }
...
function encodeRequest(rpc, protocol, servicePath, servicePort, request, numericEnums) {
    ...
    if (numericEnums) {
        transcoded.queryString = (transcoded.queryString ? "".concat(transcoded.queryString, "&") : '') + '$alt=json%3Benum-encoding=int';
    }
...
function decodeResponse(rpc, ok, response) {
    ...
    if (!message) {
        throw new Error("Received null or malformed response from JSON serializer from RPC ".concat(rpc.name));
    }
    ...
    _transform(chunk, _, callback) {
        ...
        if (this._level === 0 && curIndex === 0) {
            if (String.fromCharCode(chunk[0]) !== '[') {
                this.emit('error', new Error("Internal Error: API service stream data must start with a '[' and close with the corresponding ']', but it start with ".concat(String.fromCharCode(chunk[0]))));
            }
            curIndex++;
            this._level++;
        }
 ...
 */ class IamClient {
     ...
     */ initialize() {
        ...
        for (const methodName of iamPolicyStubMethods){
            const innerCallPromise = this.iamPolicyStub.then((stub)=>{
                var _this = this;
                return function() {
                    for(var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++){
                        args[_key] = arguments[_key];
    ...
    options){
        ...
        if (typeof __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"] !== 'undefined' && 'versions' in __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"]) {
            ...
            clientHeader.push("gl-web/".concat(version));
        }
        if (!opts.fallback) {
            clientHeader.push("grpc/".concat(gaxGrpc.grpcVersion));
        }
        if (opts.libName && opts.libVersion) {
 ...
 */ class LocationsClient {
     ...
     */ initialize() {
        ...
        for (const methodName of locationsStubMethods){
            const callPromise = this.locationsStub.then((stub)=>{
                var _this = this;
                return function() {
                    for(var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++){
                        args[_key] = arguments[_key];
    ...
    opts){
        ...
        if (!opts.fallback) {
            ...
            clientHeader.push("rest/".concat(gaxGrpc.grpcVersion));
        }
        if (opts.libName && opts.libVersion) {
            clientHeader.push("".concat(opts.libName, "/").concat(opts.libVersion));
        }
    ...
    customServicePath) {
        ...
        if (opts.universeDomain) {
            ...
            if (universeFromAuth && opts.universeDomain !== universeFromAuth) {
                throw new Error("The configured universe domain (".concat(opts.universeDomain, ") does not match the universe domain found in the credentials (").concat(universeFromAuth, "). ") + "If you haven't configured the universe domain explicitly, googleapis.com is the default.");
            }
        }
        service.resolveAll();
